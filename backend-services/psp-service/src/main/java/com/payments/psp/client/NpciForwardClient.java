package com.payments.psp.client;
...
@Component
public class NpciForwardClient {

    private final RestClient npci;
    private final ObjectMapper mapper;

    public NpciForwardClient(RestClient npciRestClient, ObjectMapper mapper) {
        this.npci = npciRestClient;
        this.mapper = mapper;
    }

    public ResponseEntity<PayResponse> pay(PayRequest body, String idempotencyKey) {
        return postWithIdempotency(ApiPaths.NPCI_BASE + ApiPaths.PAY, body, idempotencyKey, PayResponse.class);
    }

    public ResponseEntity<CollectCreatedResponse> createCollect(CollectRequest body, String idempotencyKey) {
        return postWithIdempotency(ApiPaths.NPCI_BASE + ApiPaths.COLLECT_REQUESTS, body, idempotencyKey, CollectCreatedResponse.class);
    }

    public ResponseEntity<PayResponse> approveCollect(String id, CollectApproveRequest body, String idempotencyKey) {
        String path = ApiPaths.NPCI_BASE + ApiPaths.COLLECT_APPROVE.replace("{id}", id);
    ...
    public ResponseEntity<BalanceResponse> getBalance(String vpa) {
        String path = UriComponentsBuilder.fromPath(ApiPaths.NPCI_BASE + ApiPaths.BALANCE).queryParam("vpa", vpa).build().toUriString();
    ...
    public ResponseEntity<VpaRegisterResponse> registerVpa(String vpaId, String bankCode, String accountId) {
        String path = UriComponentsBuilder.fromPath(ApiPaths.NPCI_BASE + ApiPaths.REGISTER_VPA_WITH_ID).buildAndExpand(vpaId).toUriString();
